...
import { MINTABLE_TOKEN_ADDRESS, MINTABLE_TOKEN_ABI } from './tokenfactory';
...
export function Usetokenfactory(chain: keyof typeof CHAIN_CONFIGS = 'zorasepolia') {
  ...
  const createToken = async (name: string, symbol: string) => {
    ...
    // Optionally, parse receipt for new token address
    return receipt.hash;
  };
